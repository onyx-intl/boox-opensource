...
bool OLEStorage::open( Mode mode )
{
    ...
    if ( mode == ReadOnly )
    {
        m_inputFile = new POLE::Storage(m_fileName.c_str());
        if ( !m_inputFile )
        {
            return false;
        }
        return m_inputFile->open();
    }
    else
    {
        /*
...
OLEStreamReader* OLEStorage::createStreamReader( const std::string& stream )
{
    ...
    */
    ...
    OLEStreamReader* reader( new OLEStreamReader( new POLE::Stream(m_inputFile, stream), this ) );
...
OLEStreamWriter* OLEStorage::createStreamWriter( const std::string& stream )
{
    ...
    GsfOutput* output = 0;//( gsf_outfile_new_child( currentDir, stream.c_str(), FALSE ) );
