...
void ChangeEncodingAction::run()
{
    ...
    fbreader().createDescription(fbreader().document_path.toLocal8Bit().constData(), description);
    if (description) {
        fbreader().bookTextView().gotoParagraph(0);
        WritableBookDescription desc(*description);
        desc.encoding() = fbreader().EncodingOption.value();
        fbreader().openBook(description);
        fbreader().refreshWindow();
    }
}
...
void ScrollingAction::run() {
    ...
    if (view && ((delay < 0) || (delay >= myOptions.DelayOption.value()))) {
...
bool GotoNextTOCSectionAction::isVisible() const {
    ...
    return model && (model->paragraphsNumber() > 1);
}

bool GotoNextTOCSectionAction::isEnabled() const {
    const ContentsView &contentsView = (const ContentsView&)*fbreader().myContentsView;
    shared_ptr<ZLTextModel> model = contentsView.model();
    return model && ((int)contentsView.currentTextViewParagraph() < (int)model->paragraphsNumber() - 1);
}
...
bool SelectionAction::isEnabled() const {
    ...
    return !selectionModel.text().empty() || selectionModel.image();
}
