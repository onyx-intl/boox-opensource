...
const std::vector<ZLTreeNodePtr> &FBFileHandler::subnodes() const {
	...
	if (!myIsUpToDate) {
		if (!myDir->isRoot()) {
                  mySubnodes.push_back(shared_ptr<ZLTreeNode>(new ZLTreeNode("..", "..", UpFolderIcon, true)));
		}
		...
		for (it = names.begin(); it != names.end(); ++it) {
			...
			folderNodes[displayName].reset(new ZLTreeNode(*it, displayName, FolderIcon, true));
		}
		names.clear();
		...
		for (it = names.begin(); it != names.end(); ++it) {
			...
			if (plugin != 0) {
				...
				fileNodes[displayName].reset(new ZLTreeNode(*it, displayName, icon, false));
			} else if (file.isArchive()) {
                          folderNodes[displayName].reset(new ZLTreeNode(*it, displayName, ZipFolderIcon, true));
			}
		}
