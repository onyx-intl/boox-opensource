...
ZLTextSelectionModel::ZLTextSelectionModel(ZLTextView &view, ZLApplication &application)
: myView(view)
...
, myArea(0, 0, 0, 0, false, shared_ptr<ZLTextStyle>(), ZLTextElement::WORD_ELEMENT, 0, 0, 0, 0, 0)
{
}
...
void ZLTextSelectionModel::startSelectionScrolling(bool forward) {
	if (!mySelectionScroller) {
          mySelectionScroller.reset(new ZLTextSelectionScroller(*this));
	}
...
void ZLTextSelectionModel::createData() const {
	if (!myTextIsUpToDate && !isEmpty()) {
		...
		if ((cursor == end) && !cursor.isEndOfParagraph() && !myImage) {
			const ZLTextElement &element = cursor.element();
...
void ZLTextSelectionModel::text(std::vector<std::string> & paragraphs)
{
    paragraphs.clear();
    std::string text;
    ZLTextWordCursor start = myView.startCursor();
...
bool ZLTextSelectionModel::selectFirstWord()
{
    clear();
...
bool ZLTextSelectionModel::selectNextLineWord()
{
    ZLTextElementMap::const_iterator it = myView.myTextElementMap.begin();
    for (; it != myView.myTextElementMap.end(); ++it)
    {
        if ((it->Kind == ZLTextElement::WORD_ELEMENT) && (it->YEnd > myArea.YEnd))
        {
            break;
        }
    }
    ...
    // Search again.
    int y_dist = it->YEnd - myArea.YEnd;
    int x_dist = INT_MAX;
    ZLTextElementMap::const_iterator pos = myView.myTextElementMap.end();
    for (; it != myView.myTextElementMap.end(); ++it)
    {
        if (it->Kind == ZLTextElement::WORD_ELEMENT)
        {
            if (y_dist < it->YEnd - myArea.YEnd)
            {
                break;
            }
            if (x_dist > abs(it->XStart - myArea.XStart))
            {
                pos = it;
                x_dist = abs(it->XStart - myArea.XStart);
            }
        }
    }

    if (pos == myView.myTextElementMap.end())
    {
        return false;
    }

    clear();
    return selectWord(*pos, pos->XStart, pos->YEnd);
}

bool ZLTextSelectionModel::selectPrevLineWord()
{
    int y_dist = INT_MAX;
    ZLTextElementMap::const_iterator it = myView.myTextElementMap.begin();
    for (; it != myView.myTextElementMap.end(); ++it)
    {
        if ((it->Kind == ZLTextElement::WORD_ELEMENT))
        {
            if (myArea.YEnd > it->YEnd)
            {
                if (y_dist > myArea.YEnd - it->YEnd)
                {
                    y_dist = myArea.YEnd - it->YEnd;
                }
            }
            else
            {
    ...
    if (it == myView.myTextElementMap.end() || it == myView.myTextElementMap.begin())
    {
        return false;
    }
    ...
    int x_dist = INT_MAX;
    ZLTextElementMap::const_iterator pos = myView.myTextElementMap.end();
    for (; it != myView.myTextElementMap.begin(); --it)
    {
        if (it->Kind == ZLTextElement::WORD_ELEMENT)
        {
            if (y_dist < myArea.YEnd - it->YEnd)
