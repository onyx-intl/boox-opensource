...
void ZLTextView::ViewStyle::reset() {
    // John: the line spacing is not updated after reset.
    // Not sure why it happens. I guess there may be a problem
    // when replacing the shared_ptr with boost::shared_ptr
    // Now, just force to update the myWordHeight.
    myWordHeight = -1;
	setTextStyle(ZLTextStyleCollection::instance().baseStylePtr(), myBaseBidiLevel);
}
...
void ZLTextView::ViewStyle::applyControl(const ZLTextStyleElement &control) {
  setTextStyle(
      shared_ptr<ZLTextForcedStyle>(
          new ZLTextForcedStyle(myTextStyle, control.entry())), myBidiLevel);
}
