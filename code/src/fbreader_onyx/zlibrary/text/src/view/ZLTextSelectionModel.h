...
public:
    ...
    bool selectPrevLineWord();
    void extendWordSelectionToParagraph();
    const ZLTextElementArea & wordArea() const { return myArea; }

    void text(std::string & text, int max);
    void text(std::vector<std::string> & paragraphs);
