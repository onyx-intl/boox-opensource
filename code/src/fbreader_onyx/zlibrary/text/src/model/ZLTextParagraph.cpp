	...
	if (!myEntry) {
		switch (*myPointer) {
			case ZLTextParagraphEntry::TEXT_ENTRY:
                          myEntry.reset(new ZLTextEntry(myPointer + 1));
				break;
			...
			case ZLTextParagraphEntry::HYPERLINK_CONTROL_ENTRY:
                          myEntry.reset(new ZLTextHyperlinkControlEntry(myPointer + 1));
				break;
			case ZLTextParagraphEntry::IMAGE_ENTRY:
			{
				...
				myEntry.reset(new ImageEntry(myPointer + sizeof(const ZLImageMap*) + sizeof(short) + 1, imageMap, vOffset));
				break;
			}
			case ZLTextParagraphEntry::STYLE_ENTRY:
                          myEntry.reset(new ZLTextStyleEntry(myPointer + 1));
				break;
			case ZLTextParagraphEntry::FIXED_HSPACE_ENTRY:
                          myEntry.reset(new ZLTextFixedHSpaceEntry((unsigned char)*(myPointer + 1)));
				break;
...
shared_ptr<const ZLImage> ImageEntry::image() const {
	...
	return (it != myMap->end()) ? (*it).second : shared_ptr<const ZLImage>();
}
