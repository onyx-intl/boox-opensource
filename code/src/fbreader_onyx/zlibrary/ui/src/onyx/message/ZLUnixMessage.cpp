...
NabooCommunicationManager::NabooCommunicationManager()
{
}

void NabooCommunicationManager::createInstance()
{
    if (ourInstance == 0)
    {
        ourInstance = new NabooCommunicationManager();
    }
}

shared_ptr<ZLMessageOutputChannel> NabooCommunicationManager::createMessageOutputChannel(const std::string &protocol, const std::string &testFile) {
    if (protocol != "execute") {
        return 0;
    }
