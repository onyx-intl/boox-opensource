...
void ZLQtFSManager::normalize(std::string &path) const
{
    ...
    path = info.absoluteFilePath().toLocal8Bit().constData();
}

std::string ZLQtFSManager::resolveSymlink(const std::string &path) const
{
    QFileInfo info(QString::fromLocal8Bit(path.c_str()));
    return info.absoluteFilePath().toLocal8Bit().constData();
}
...
ZLFSDir *ZLQtFSManager::createNewDirectory(const std::string &path) const
{
    ...
    {
        return createPlainDirectory(dir.absolutePath().toLocal8Bit().constData());
    }
    return 0;
}
...
{
    return shared_ptr<ZLDir>(static_cast<ZLDir*>(createPlainDirectory(
        QDir::root().absolutePath().toLocal8Bit().constData())));
}

const std::string &ZLQtFSManager::rootDirectoryPath() const
{
    static const std::string root = QDir::root().absolutePath().toLocal8Bit().constData();
    return root;
}
