...
void ZLSplittedView::setLeftView(shared_ptr<ZLView> view) {
	myLeftView = view;
	if (myLeftView && hasContext()) {
		myLeftView->setPaintContext(
                    shared_ptr<ZLPartialPaintContext>(
                        new ZLPartialPaintContext(*this, true)));
	}
}

void ZLSplittedView::setRightView(shared_ptr<ZLView> view) {
	myRightView = view;
	if (myRightView && hasContext()) {
          myRightView->setPaintContext(
              shared_ptr<ZLPartialPaintContext>(new ZLPartialPaintContext(*this, false)));
	}
}
...
bool ZLSplittedView::onStylusPress(int x, int y) {
	...
	if ((x < leftW) && myLeftView) {
		return myLeftView->onStylusPress(x, y);
	}

	if ((x > leftW + splitterWidth() + 1) && myRightView) {
