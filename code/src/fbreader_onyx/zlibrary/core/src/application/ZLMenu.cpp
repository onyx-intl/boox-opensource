...
void ZLMenu::addItem(const std::string &actionId, const ZLResourceKey &key) {
  myItems.push_back(
      shared_ptr<ZLMenubar::PlainItem>(
          new ZLMenubar::PlainItem(myResource[key].value(), actionId)));
}

void ZLMenu::addSeparator() {
	myItems.push_back(
            shared_ptr<ZLMenubar::Separator>(new ZLMenubar::Separator()));
}
