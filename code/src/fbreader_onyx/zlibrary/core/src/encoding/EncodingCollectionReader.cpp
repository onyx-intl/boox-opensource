...
void ZLEncodingCollectionReader::startElementHandler(const char *tag, const char **attributes) {
	if (GROUP == tag) {
		...
		if (name != 0) {
                  myCurrentSet.reset(new ZLEncodingSet(name));
		}
	} else if (myCurrentSet) {
		if (ENCODING == tag) {
			const char *name = attributeValue(attributes, NAME.c_str());
			const char *region = attributeValue(attributes, REGION.c_str());
			if ((name != 0) && (region != 0)) {
				const std::string sName = name;
				myCurrentInfo.reset(new ZLEncodingConverterInfo(sName, region));
				myNames.push_back(sName);
			}
		} else if (myCurrentInfo) {
			const char *name = 0;
			if (CODE == tag) {
...
void ZLEncodingCollectionReader::endElementHandler(const char *tag) {
	if (myCurrentInfo && (ENCODING == tag)) {
		...
		myCurrentInfo.reset();
		myNames.clear();
	} else if (myCurrentSet && (GROUP == tag)) {
