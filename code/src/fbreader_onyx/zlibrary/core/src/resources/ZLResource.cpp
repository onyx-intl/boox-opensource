...
void ZLTreeResource::buildTree() {
	if (!ourRoot) {
          ourRoot.reset(new ZLTreeResource(std::string()));
		loadData("en");
...
const ZLMissingResource &ZLMissingResource::instance() {
	if (!ourInstance) {
          ourInstance.reset(new ZLMissingResource());
	}
	return *ourInstance;
}
...
void ZLResourceTreeReader::startElementHandler(const char *tag, const char **attributes) {
	if (!myStack.empty() && (NODE == tag)) {
		...
		if (name != 0) {
			...
			if (!node) {
				if (value != 0) {
                                  node.reset(new ZLTreeResource(sName, value));
				} else {
                                  node.reset(new ZLTreeResource(sName));
				}
