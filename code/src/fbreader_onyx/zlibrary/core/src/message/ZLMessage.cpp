...
public:
  shared_ptr<ZLMessageOutputChannel> createMessageOutputChannel(const std::string&, const std::string&) { return shared_ptr<ZLMessageOutputChannel>(); }
...
shared_ptr<ZLMessageHandler> ZLCommunicationManager::handler(const std::string &command) {
	...
	if (it == myRegisteredHandlers.end()) {
          return shared_ptr<ZLMessageHandler>();
	}
	if (it->second.lock()) {
