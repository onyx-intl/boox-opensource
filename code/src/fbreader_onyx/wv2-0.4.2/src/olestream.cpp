...
bool OLEStreamReader::seek( int offset, GSeekType whence )
{
    ...
    case G_SEEK_SET:
        ...
        break;
    case G_SEEK_CUR:
        m_stream->seek(m_stream->tell() + offset);
        break;
    case G_SEEK_END:
...
#else
    U16 ret;
    m_stream->read(reinterpret_cast<unsigned char *>(&ret), sizeof(ret));
    return ret;
#endif
}
...
OLEStreamWriter::OLEStreamWriter( POLE::Stream *  stream, OLEStorage* storage ) :
    OLEStream( storage ), m_stream( stream )
{
}
...
POLE::Stream* OLEStreamWriter::stream()
{
    return m_stream;
}
