...
bool DictData::open(const QString& fname, int computeCRC)
{
    struct stat sb;
    int j;
    int fd;
    ...
    if (read_header(fname.toStdString(), computeCRC))
    {
    ...
    if ((fd = ::open(fname.toStdString().c_str(), O_RDONLY )) < 0)
    {
        //err_fatal_errno( __FUNCTION__,
    ...
    ::close(fd);
    mapfile.setFileName(fname);
    if (!mapfile.open(QIODevice::ReadOnly))
        return false;

    this->start = reinterpret_cast<char*>(mapfile.map(0, mapfile.size()));
...
void DictData::close()
{
    ...
    mapfile.unmap(reinterpret_cast<uchar *>(const_cast<char *>(this->start)));
    mapfile.close();
}
